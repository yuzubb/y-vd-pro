...
from Cogs.db import get_paypay_account, is_admin
import paypayu
...
async def process_paypay(interaction, paypay_link, price, bot):
    ...
    # Supabaseからオーナーのアカウントを取得（再起動後も永続）
    owner_account = get_paypay_account(bot.owner_id)
...
class AccountVendingCog(commands.Cog):
    ...
    @app_commands.command(name="アカウント販売機", description="アカウント自動販売機")
    @is_allowed()
    ...
    @app_commands.command(name="テストアカウント販売機", description="テスト用アカウント自動販売機（管理者のみ）")
    async def test_account_vending(self, interaction: discord.Interaction):
        ...
        for name, label in [("full", "代行全適用"), ("copy", "複製"), ("restore", "復旧"), ("new", "新規作成")]:
            embed.add_field(name=label, value="無料", inline=True)
        ...
        for label, cb, style in [
            ("代行全適用アカウント", full_cb, discord.ButtonStyle.primary),
            ("アカウント複製", copy_cb, discord.ButtonStyle.secondary),
            ("アカウント復旧", restore_cb, discord.ButtonStyle.success),
            ("新規アカウント作成", new_cb, discord.ButtonStyle.secondary),
        ]:
