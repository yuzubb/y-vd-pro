...
def _load_allowed_users_json():
    """JSONファイルから許可ユーザーIDリストを読み込む（フォールバック用）"""
...
def is_allowed():
    """権限チェック。Supabaseが使える場合はDBを参照し、なければJSONにフォールバック。"""
    async def predicate(interaction: discord.Interaction) -> bool:
        if await interaction.client.is_owner(interaction.user):
            return True

        # Supabase経由チェック（db.pyが使える場合）
        try:
        ...
        # JSONフォールバック
        allowed_ids = _load_allowed_users_json()
