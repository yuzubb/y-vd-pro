...
PROXY_URL = None  # プロキシを使う場合は "http://..." を設定
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.post(
                ...
                json=receive_payload, headers=auth_headers, proxy=PROXY_URL
            ) as response:
                response.raise_for_status()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            for endpoint in send_endpoints:
                try:
                    async with session.post(endpoint, headers=send_headers, json=send_payload, proxy=PROXY_URL) as response:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            for endpoint in request_endpoints:
                try:
                    async with session.post(endpoint, headers=request_headers, json=request_payload, proxy=PROXY_URL) as response:
                        if response.status == 200:
